...
function BusinessForm() {
  ...
  useEffect(() => {
    if (isTrueBiz?.images?.length > 0) {
      const bizData = isTrueBiz.images[0].bizLicense?.result;
      ...
      // 데이터가 존재하는지 확인 후 처리
      const registerNumber = bizData.registerNumber?.[0]?.text || "";
      const openDate = bizData.openDate?.[0]?.text || "";
      const repName = bizData.repName?.[0]?.text || "";
      const companyName = bizData.companyName?.[0]?.text || "";

      if (!registerNumber || !openDate || !repName || !companyName) {
        alert("사업자 등록증 정보를 확인할 수 없습니다.");
        return;
      }

      const bizNumber = registerNumber.replace(/\D/g, "");
      const bizopenDate = openDate.replace(/\D/g, "");
      const bizName = repName;
      const bizShopName = companyName;
      ...
      const isBizEnter = async () => {
        try {
          const res = await axios.post(
            ...
            {
              businesses: [
                { b_no: bizNumber, start_dt: bizopenDate, p_nm: bizName },
              ],
            }
          );
  ...
  useEffect(() => {
    const fetchTerms = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/api/users/terms`
        );
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/email/verify`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const checkDuplicate = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/users/${formData.userId}`
      );
  ...
  const validateForm = () => {
    ...
    if (!formData.userPassword) {
      ...
      errors.userPassword =
        "비밀번호는 8~16자리, 영문/숫자/특수기호를 포함해야 합니다";
    }
    ...
    const allRequiredChecked = requiredTerms.every(
      (term) => checkedItems[term.termsId]
    );
  ...
  const handleSubmit = async () => {
    try {
      const optionalTerms =
        terms
          .filter((term) => term.isRequired === "F")
          .map((term) => checkedItems[term.termsId] === true)[0] || false;
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/users/sign-up/biz`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800 mb-24">
      <div className="flex mx-auto max-w-xl flex-col">
        {/* 폼 제목 */}
        <p className="text-2xl font-bold mt-24 mb-8 text-center">
          착한업소 솔루션 기업회원 가입
        </p>
              ...
              {tooltipVisible && (
                ...
                >
                  ...
                  <p className="text-gray-600 text-sm pt-2">
                    올바른 사업자 등록증 파일인지 확인 후 다시 업로드해 주세요.
                    초기화 버튼을 클릭하면 기업 인증을 다시 진행할 수 있습니다.
                  </p>
        ...
        <div className="relative mb-3">
          ...
          <span className="label">
            {formErrors.userId && (
              <span className="text-red-500">{formErrors.userId}</span>
            )}
          </span>
        </div>
        ...
        <div className="relative mb-3">
          ...
          <span className="label">
            {formErrors.userBirth && (
              <span className="text-red-500">{formErrors.userBirth}</span>
            )}
          </span>
        </div>
        ...
        <div className="accordion divide-neutral/20 divide-y rounded border p-5 mb-10">
          ...
          <div className="space-y-2 mb-5">
            ...
            <p className="text-sm text-gray-500 ml-7">
              실명 인증된 아이디로 가입, 위치기반서비스 이용약관(필수),

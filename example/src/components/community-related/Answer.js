...
const Answer = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                if (data.notice === '공지사항') {
                    const res = await axios.get(`http://localhost:8080/notice/main/${data.id}`);
                    setEditingMain(res.data);
                    const attachments = await axios.get(`http://localhost:8080/notice/attachment/${data.id}/download`);
                    setFiles(attachments.data);
                }
                if (data.qna === 'Q&A') {
                    const res = await axios.get(`http://localhost:8080/qna/main/${data.id}`);
                    setEditingMain(res.data);
                ...
                if (mainPageNotice) {
                    const res = await axios.get(`http://localhost:8080/notice/main/${mainPageNotice.noticeId}`);
                    setEditingMain(res.data);
    ...
    const handleSaveMain = async () => {
        try {
            //notice qna로직 구분
            const res = await axios.patch(`http://localhost:8080/notice/update/${data.id}`, {
    ...
    const handleSaveEdit = async () => {
        try {
            const res = await axios.patch(`http://localhost:8080/qna/update/answer/${editingAnswerId}`, {

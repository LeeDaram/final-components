...
const Write = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (data?.notice) {
            try {
                const noticeResponse = await axios.post(`http://localhost:8080/notice/create`, {
                    title: title,
                    content: content,
        ...
        if (data?.notice && selectedFiles.length > 0 && noticeId) {
            ...
            try {
                const uploadResponse = await axios.post('http://localhost:8080/notice/attachments', formData);

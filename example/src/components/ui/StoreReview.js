...
function Storereview({ data }) {
    ...
    const fetchReviews = async (page) => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/store/reviews/${data}?sort=${sort}`, {
            ...
            response.data.content.forEach((item) => {
                newImgNumList[item.reviewId] = item.imageIds ? item.imageIds.split(',') : [];
            });
    ...
    const toggleLike = async (reviewId) => {
        ...
        setReview((prevReviews) =>
            prevReviews.map((review) =>
                review.reviewId === reviewId ? { ...review, likeCount: review.likeCount + (isLiked ? 1 : -1) } : review
            )
        );
                ...
                review.map((v, index) => (
                    <>
                        <div key={`review-${index}`} className=" pt-4">
                            ...
                            {/* 작성일 */}
                            <p className="text-xs text-gray-500">{formatDate(v.createdAt)}초</p>
                            ...
                            <div className="flex items-center mt-2">
                                <div className="text-black text-xl">이 리뷰가 마음에 들으셨나요?</div>
                                <button
                                    onClick={() => toggleLike(v.reviewId)}
                                    className={`w-20 ml-3 flex items-center px-3 py-1 border rounded-full transition-all 
    ${likedReviews[v.reviewId] === 1 ? 'border-red-500 text-red-500' : 'border-gray-300 text-gray-500'} 
    hover:border-gray-400`}
                                >
                                    <FaRegThumbsUp
                                        className={`w-5 h-5 mr-1 ${
                                            likedReviews[v.reviewId] === 1 ? 'fill-red-500 text-white' : ''
                                        }`}
                                    />

...
import { MdOutlineTakeoutDining, MdOutlineDeliveryDining } from 'react-icons/md'; // 포장 아이콘, 배달 아이콘
...
import kakaoImg from '../../assets/images/store/KakaoTalk.png';
...
const StoreDetail = () => {
    ...
    const handleReservation = async () => {
        ...
        try {
            const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/reservation`, requestData);
        ...
        // 데이터 없을경우 화면 처리
        if (!store) return <span className="loading loading-spinner loading-lg"></span>;
                ...
                <div className="col-span-2 flex flex-col gap-2">
                    <div className="bg-gray-300 w-full h-72 rounded-md">
                        {store.storeImage == null ? (
                            <img src={kaImg} alt="업소 대표 이미지" className="w-full h-full object-cover rounded-md" />
                        ) : (
                            <img
                ...
                <div className="border border-gray-200 p-4 rounded-md space-y-5">
                    ...
                    <div className="flex flex-col gap-2 ">
                        <p className="text-sm text-gray-600">
                            {' '}
                            <span className="text-black font-bold">주소</span>: {store.address}
                        </p>
                        <p className="text-sm text-gray-600">
                            <span className="text-black font-bold">전화번호</span>: {store.contact}
                        </p>
                        <p className="text-sm text-gray-600">
                            <span className="text-black font-bold">업종</span>: {store.industryName}
                        </p>
                        {/* 예약버튼*/}
                        <div className="flex">
                            <button
                        ...
                        <div className="flex items-center">
                            <div className="p-1 bg-blue-500 text-white rounded text-sm">착한가격</div>
                            <div className="text-gray-400 text-sm pl-1 text-center">{store.mainMenu}</div>
                ...
                {modalOn && (
                    ...
                    >
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-lg">
                            ...
                            <Rating name="size-medium" defaultValue={1} onClick={(e) => setRating(e.target.value)} />
                            {/* 사진 */}
                            <p className="text-blue-500 text-xl mt-4">사진</p>
                            <div className="flex items-center gap-3">
                                {/* 아이콘 클릭시 사진 업로드 실행 */}
                                <CiImageOn className="cursor-pointer" size={40} onClick={() => triggerFileUpload()} />
                                {/* 숨겨진 파일 업로드 */}
                                <input
                                    type="file"
                                    multiple
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {previews.length > 2 && <p className="text-base font-bold">+{images.length - 2}</p>}
            ...
            <div className="flex w-full mt-6">
                ...
                <div className="w-1/2 flex flex-col items-center">
                    ...
                    <div className="flex mt-2 space-x-2 font-medium">
                        <p className="text-gray-500">
                            <span className="text-blue-500">{store.likeCount.toLocaleString()}명</span>이
                            <span className="text-blue-500"> {store.storeName}</span>
                            을(를) 좋아합니다.
                        </p>
                    </div>
                </div>
                ...
                <div className="w-1/2 flex items-center">
                    <div className="flex flex-col items-center w-1/2">
                        <p className="text-sm font-bold text-gray-800 flex items-center">총 {store.reviewCount}건</p>
                        <p className="text-xl font-bold text-gray-800 flex items-center">{store.reviewAvg}점</p>
                        <Rating name="half-rating-read" defaultValue={store.reviewAvg} precision={0.5} readOnly />
                    </div>
                    {/* 평점 비율2 */}
                    <div className="flex flex-col w-1/2 gap-2">
                        {ratingData.map((item) => (
                            <div className="flex items-center gap-2" key={`rating-${item.rt}-${item.value}`}>
                                <div
                                ...
                                <span className="text-gray-800 font-medium w-10 text-right">{item.rt}</span>
                            </div>
                        ))}
                    </div>
                </div>
